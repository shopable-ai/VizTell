...
CONTACT=re.compile(
    rf'(?i)(?:[（(]\s*)?(?:(?:更多[^。！？\n]{{0,24}}[,，]?\s*)?(?:添加(?:作者)?|加作者)\s*)?(?:拼课\s*)?(?:我的\s*)?(?:微信号|微信|微芯|薇芯|薇信|V信|VX|WX)\s*[:：号]?\s*{ACCOUNT}(?:\s*[）)])?'
)
...
def clean_text(text:str)->tuple[str,dict]:
    # Run on the full text so OCR line breaks between `微信：` and the account
    # are still matched.  Do not normalize whitespace or delete unrelated
    # punctuation/empty brackets: only explicit promo tokens are removed.
    counts={'contact':0,'public_watermark':0,'more_materials':0}
    out,n=CONTACT.subn('',text); counts['contact']+=n
    out,n=PUBLIC.subn('',out); counts['public_watermark']+=n
    out,n=MORE.subn('',out); counts['more_materials']+=n
    return out,counts
...
def main()->int:
    ...
    for path in sorted(Path('temp').rglob('*.md')):
        ...
        for kind,pat in [('contact',CONTACT),('public_watermark',PUBLIC),('more_materials',MORE)]:
            m=pat.search(text)
            if m:
                remaining.append({'path':str(path),'kind':kind,'match':m.group(0)[:120]})
                break
    if remaining:
        errors.append({'remaining_explicit_promo':remaining})
