...
for item in data.get('review', []):
    ...
    for issue in item.get('issues', []):
        ...
        rows.append(f"DEFECT\t{code}\t{path}\t{detail[:500]}")
        by_code[code].append(path)

candidate_by_code = defaultdict(list)
for item in data.get('review_candidates', []):
    path = item.get('path')
    for issue in item.get('candidates', []):
        code = issue.get('code')
        detail = str(issue.get('detail', '')).replace('\t', ' ').replace('\n', ' ')
        rows.append(f"CANDIDATE\t{code}\t{path}\t{detail[:500]}")
        candidate_by_code[code].append(path)

out.write_text('\n'.join(rows) + ('\n' if rows else ''), encoding='utf-8')
compact = {
    'audit_summary': data.get('summary', {}),
    'issue_types': {
        code: {'count': len(paths), 'paths': sorted(set(paths))}
        for code, paths in sorted(by_code.items())
    },
    'review_candidate_types': {
        code: {'count': len(paths), 'paths': sorted(set(paths))}
        for code, paths in sorted(candidate_by_code.items())
    },
}
summary_out.write_text(json.dumps(compact, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
print(json.dumps({
    'defects': {code: len(paths) for code, paths in sorted(by_code.items())},
    'review_candidates': {code: len(paths) for code, paths in sorted(candidate_by_code.items())},
}, ensure_ascii=False))
