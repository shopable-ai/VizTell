...
ROLE = r"(?:作者|本书作者|客服|老师|讲师|助理|小编|班主任|顾问|私人)"
...
PROMO_PREFIX = re.compile(
    rf"(?i)^[—\-–~·•\s]*更多精品(?:课程|资料|资源|电子书)\s*"
    rf"(?:加|添加)(?:微\s*信|威信|V信|VX)\s*[：:=]?\s*(?:【)?{CRED}(?:】)?"
)
...
def direct_ad(line: str) -> bool:
    ...
    if not s:
        return False
    credential = CRED_CONTACT.search(s)
    credential_is_ad = bool(credential and (
        len(s) <= 100 or re.search(r"(?:添加|联系|客服|作者|老师|领取|获取|更多|扫码|关注|资源|资料|课程)", s)
    ))
    return bool(
        ...
        or PROMO_PREFIX.search(s)
    )


def strip_prefix(line: str) -> tuple[str, bool]:
    """Remove a known promotional prefix while preserving attached正文."""
    m = PROMO_PREFIX.match(unwrap(line))
    if not m:
        return line, False
    plain = unwrap(line)
    rest = plain[m.end():].lstrip(" ：:，,。；;、|-—")
    return rest, True
...
def clean_text(text: str) -> tuple[str, dict]:
    ...
    for n, raw in enumerate(text.splitlines(), 1):
        ...
        new_line, prefix_changed = strip_prefix(line)
        if prefix_changed:
            prefixes.append({"line": n, "before": line[:260], "after": new_line[:220]})
            line = new_line
            if not line.strip():
                neighbor_budget = 2
                continue
    ...
    changed_any = bool(removed or prefixes or tails or shifts)
    if not changed_any:
        return text, {"removed": [], "prefixes": [], "tails": [], "heading_shifts": []}
    result = "\n".join(out)
    if text.endswith("\n") and not result.endswith("\n"):
        result += "\n"
    if Counter(IMG.findall(result)) != images:
        raise RuntimeError("Markdown image references changed")
    return result, {"removed": removed, "prefixes": prefixes, "tails": tails, "heading_shifts": shifts}
...
- 优先删除的高置信广告包括：明确要求“添加 / 联系 / 扫码添加”作者、客服、老师微信，明确账号，领取更多资料 / 课程 / 电子书的第三方引流，以及关注“本公众号”的书外宣传；仅出现“老师的微信”“微信群”等叙述不算广告；
- 如果固定广告串被直接粘在正文句首（例如“更多精品课程加威信xxxx”后面紧接正文），只剥离广告前缀，后面的正文必须完整保留；
...
def main() -> int:
    ...
    report = {
        ...
        "summary": {
            ...
            "ad_prefixes_removed": sum(x["ad_prefixes_removed"] for x in results),
