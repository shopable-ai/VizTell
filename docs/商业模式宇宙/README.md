...
| `10_任务工作流与运行时` | 成熟度地图、任务宇宙、Task Router、Retrieval、Data/Evidence Gate、端到端 Workflow、Trace 与知识生命周期 |
...
- **真实商业任务（推荐）**：`10/00 成熟度与优先级 → 10/01 Task Router/检索/数据门 → 10/02 Workflow → 按路由加载 01/02/03/06/09 → 11 Benchmark/Regression`
- **设计真实项目**：`10 运行时 → 01 核心机制/商品角色/Offer（按需）→ 02 价值链 → 03 生态 → 06 真实经济性 → 05 失败/迁移 → 04 案例/行业`
- **AI / Agent / Skill**：`00/00_当前有效版本与加载入口 → 07_元数据知识图谱与Skill → 08_机器数据与Schema → 10 运行时 → 11 Benchmark`
