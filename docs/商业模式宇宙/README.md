...
→ 05 P171-P200 人类阅读镜像
→ 10 结构化知识地图
→ 20 机会来源、差异捕获与商业模式变换算子
```

其中：

- `05_v4新增组合模式P171-P200.md` 是机器 P171-P200 JSONL 的人类阅读镜像；
- `20_机会来源_差异捕获与商业模式变换算子.md` 是横向机会发现与重构镜头，不是 U28，也不新增 atom/pattern ID。

## 推荐路径

- **快速理解**：`README → 00/00_当前有效版本与加载入口 → 01/00 总览 → 01/04 商品角色/价格/Offer → 01/05 P171-P200 → 01/20 机会来源/变换算子 → 06 真实经济性`
- **发现商业机会**：`01/20 机会来源/差异/错配 → 变换算子 → 01/01 U01-U27 原子 → 01/02/03/05 Pattern → 02 利润池 → 03 生态 → 06 真实经济性 → 05 失败/迁移`
...
- 并发重构产生的重复机会来源与 Pattern 过渡路径。当前稳定入口为 **05=P171-P200、20=机会来源/变换算子**。

完整性门见 `00_导航与总览/13_完整性与反方审计清单.md`，机器基线见 `08_机器数据与Schema/validation_v4.json`。被淘汰的并发过渡版本记录在 `90_研究来源与参考/99_历史结构/机会来源并发重构版本说明.md`。
