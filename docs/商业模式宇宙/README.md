...
| `01_核心机制与组合模式` | 当前总览、U01-U27、L2 atoms、L3 patterns、v4 商品角色/价格/Offer、机会来源与变换算子、P171-P200 人类阅读镜像、主题知识地图 |
...
→ 03 P131-P170
→ 04 v4 商品角色/价格/Offer
→ 05 机会来源、差异捕获与商业模式变换算子
→ 06 P171-P200 人类阅读镜像
→ 10 结构化知识地图
```

`05_机会来源_差异捕获与商业模式变换算子.md` 是横向机会发现与重构镜头，不是 U28，也不新增 atom/pattern ID；`06_v4新增组合模式P171-P200.md` 是机器 P171-P200 JSONL 的人类阅读镜像。

## 推荐路径

- **快速理解**：`README → 00/00_当前有效版本与加载入口 → 01/00 总览 → 01/04 商品角色/价格/Offer → 01/05 机会来源/变换算子 → 01/06 P171-P200 → 06 真实经济性`
- **发现商业机会**：`01/05 机会来源/差异/错配 → 变换算子 → 01/01 U01-U27 原子 → 01/02/03/06 Pattern → 02 利润池 → 03 生态 → 06 真实经济性 → 05 失败/迁移`
...
- 并发新增机会来源/变换算子后出现两个 `05_` 文件，现统一为“机会来源 05、P171-P200 06”。
...
3. 横向问题优先复用现有 U01-U27，不为“商品角色/季节/天气/Offer/机会来源”机械新增一级域。
4. 商品角色、机会来源、变换算子、定价机制、组合模式、实例必须分开建模。
